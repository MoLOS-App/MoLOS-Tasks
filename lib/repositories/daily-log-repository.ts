...
export class DailyLogRepository extends BaseRepository {
	...
	async getByDate(userId: string, logDate: number): Promise<DailyLog | null> {
		const result = await this.db
			.select()
			.from(tasksDailyLog)
			.where(and(eq(tasksDailyLog.userId, userId), eq(tasksDailyLog.logDate, logDate)))
			.limit(1);
	...
	async create(log: Omit<DailyLog, 'id' | 'createdAt' | 'updatedAt'>): Promise<DailyLog> {
	...
	async update(
		userId: string,
		logDate: number,
		updates: Partial<Omit<DailyLog, 'id' | 'userId' | 'logDate' | 'createdAt' | 'updatedAt'>>
